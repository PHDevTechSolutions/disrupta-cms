...
function SidebarSection({
  ...
  return (
    <div className="space-y-3 py-4 border-b border-slate-100 last:border-b-0">
        ...
        {items.map((item) => (
          <div key={item.id} className="flex items-center gap-2 group">
            <Checkbox
              ...
              onCheckedChange={() => onCheck(item.name)}
              disabled={!multiSelect && selected.length > 0 && !selected.includes(item.name)}
            />
      ...
      {isDynamic && onDeleteSection && (
        <Button
          variant="ghost"
          size="sm"
          className="w-full h-7 text-[9px] text-red-500 hover:text-red-700"
...
export default function AddProductForm({
  ...
  const handlePublish = async () => {
    if (!productName || (!mainImage && !existingMainImage))
      return toast.error("Required fields missing!");
    setIsPublishing(true);
    const publishToast = toast.loading(
      editData ? "Updating Product..." : "Publishing Product..."
    );
    ...
    try {
      ...
      const newlyUploadedGallery = await Promise.all(
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-4 bg-white max-h-[90vh] overflow-y-auto">
      {/* LEFT COLUMN */}
        ...
        <Card className="shadow-sm border-none ring-1 ring-slate-200">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Input
                className="h-12 text-sm border-slate-200"
                value={shortDesc}
                onChange={(e) => setShortDesc(e.target.value)}
                placeholder="Short highlight of the product..."
              />
            </div>
            ...
            <div className="space-y-4">
              {descBlocks.map((block, bIdx) => (
                <div
                  key={block.id}
                  className="p-5 border-2 border-slate-100 rounded-2xl relative bg-white shadow-sm"
                >
                  <Input
                    className="mb-4 h-8 text-[11px] font-black uppercase w-1/2 bg-slate-50 border-none"
                    value={block.label}
                    onChange={(e) => {
                      const nb = [...descBlocks];
                  ...
                  <div className="space-y-2">
                    {block.rows.map((row, rIdx) => (
                      <div key={rIdx} className="grid grid-cols-12 gap-2">
                        <Input
                          className="col-span-5 h-9 text-xs font-bold"
                          value={row.name}
                          onChange={(e) => {
                            const nb = [...descBlocks];
                        ...
                        <button
                          onClick={() => {
                            const nb = [...descBlocks];
                            nb[bIdx].rows = nb[bIdx].rows.filter(
                              (_, i) => i !== rIdx
                            );
                            setDescBlocks(nb);
                          }}
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      className="w-full text-[10px] font-bold border-dashed border-2 bg-transparent"
                      onClick={() => {
                        const nb = [...descBlocks];
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100">
              <Label className="text-[11px] font-black uppercase text-slate-500">
                Catalogs & Manuals (PDF / Images)
              </Label>
              <div className="grid grid-cols-1 gap-2">
                {catalogs.map((cat, index) => (
                  <div
                    key={cat.id}
                    className="border-2 border-dashed rounded-xl p-4 bg-slate-50 flex items-center justify-between group"
                  >
                    <label className="cursor-pointer flex-1">
                      {cat.file ? (
                        <p className="text-xs font-bold text-green-600 flex items-center gap-2">
                          <UploadCloud size={14} /> {cat.file.name}
                        </p>
                      ) : cat.existingUrl ? (
                        <a
                          href={cat.existingUrl}
                          target="_blank"
                          rel="noreferrer"
                          className="text-xs font-bold text-blue-600 underline"
                        >
                          View Current File ({index + 1})
                        </a>
                      ) : (
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                {existingGalleryImages.map((url, index) => (
                  <div
                    key={`ex-${index}`}
                    className="relative border-2 border-dashed rounded-xl p-1 bg-slate-50 h-28 group"
                  >
                    <img
                ...
                {galleryImages.map((img, index) => (
                  <div
                    key={`new-${index}`}
                    className="relative border-2 border-dashed rounded-xl p-1 bg-blue-50 h-28"
                  >
                    <img
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4">
              ...
              <div className="space-y-1">
                ...
                <Input
                  className="h-10 text-xs font-bold text-red-500"
                  value={salePrice}
      ...
      <div className="space-y-6">
        ...
        <Card className="border-none ring-1 ring-slate-200 overflow-hidden shadow-sm">
          <CardHeader className="bg-slate-50/50 border-b py-3 text-center">
            ...
            <Label htmlFor="main-file" className="cursor-pointer">
              <div className="aspect-square border-2 border-dashed rounded-2xl flex flex-col items-center justify-center hover:bg-blue-50/30 transition-all overflow-hidden bg-white border-slate-200">
                {mainImage ? (
                  ...
                  <img
                    src={existingMainImage || "/placeholder.svg"}
                    alt="Existing"
                    className="w-full h-full object-contain p-2"
                  />
                ) : (
                  <div className="text-center">
                    <ImagePlus className="w-12 h-12 mb-2 text-blue-500 mx-auto opacity-30" />
          ...
          <CardContent className="pt-4">
            <div className="space-y-4 max-h-96 overflow-y-auto">
              <SidebarSection
                ...
                onCheck={(cat) => setSelectedCats([cat])}
                val={newCat}
                setVal={setNewCat}
                onAdd={() =>
                  handleQuickAdd("categories", newCat, setNewCat)
                }
              ...
              <SidebarSection
                ...
                onCheck={(brand) => setSelectedBrands([brand])}
                val={newBrand}
                setVal={setNewBrand}
              ...
              <SidebarSection
                ...
                onCheck={(web) => setSelectedWebs([web])}
                val={newWeb}
                setVal={setNewWeb}
                onAdd={() => handleQuickAdd("websites", newWeb, setNewWeb)}
                onDelete={(id) => handleDeleteItem("websites", id)}
              />
              ...
              {customSections.map((section) => (
                <SidebarSection
                  ...
                  setVal={setNewCat}
                  onAdd={() =>
                    handleAddChoiceToCustom(section.id, newCat)
                  }
                  onDelete={(id) =>
                    handleDeleteItem("custom_sections", section.id)
                  }
                  customAddLogic={(val) =>
                    handleAddChoiceToCustom(section.id, val)
                  }
                  isDynamic
                />
              ))}
            ...
            {!isAddingNewSection && (
              <Button
                ...
                className="w-full mt-4 text-[10px] border-dashed"
              >
                + Add Custom Section
              </Button>
            )}
        ...
        <Button
          ...
          className="w-full h-12 bg-blue-600 hover:bg-blue-700 text-white font-black uppercase text-[10px] tracking-widest rounded-xl shadow-lg shadow-blue-200"
        >
